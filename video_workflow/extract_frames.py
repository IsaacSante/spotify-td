...
def _extract_video_frames(
    ...
    if existing and not force:
        print(f"  Skip {video_path.name} ({len(existing)} frames exist)")
    ...
    if duration <= tail_skip:
        print(f"  Skip {video_path.name}: {duration:.1f}s ≤ {tail_skip}s tail skip")
        cap.release()
        return 0, 0
    ...
    while t <= duration - tail_skip:
        ...
        ret, frame = cap.read()
        if not ret:
            print(f"  [WARN] Read failed at {video_path.name} @ {t}s")
            break
...
def extract_frames(
    ...
    print(f"Interval: {interval}s | Tail skip: {tail_skip}s | Similarity threshold: {sim_threshold}")
    ...
    for vf in videos:
        ...
        try:
            ...
            dur_info = f" ({n / fps:.0f}s)" if fps else ""
            cap.release()
        except Exception:
            pass

        print(f"  [{vid_hash}] {vf.name}{dur_info}")
        saved, skipped = _extract_video_frames(
            vf, vid_hash, frames_dir, interval, tail_skip, sim_threshold, force,
        )
        print(f"    → {saved} saved, {skipped} skipped (too similar)")
        total_saved += saved
        total_skipped += skipped

    print(f"Total: {total_saved} frames saved, {total_skipped} skipped → {frames_dir}/")
...
if __name__ == "__main__":
    ...
    extract_frames(
        ...
        args.tail_skip, args.sim_threshold, args.force,
    )